...
PR 상단 checks 영역에는 `GITHUB_CHECK_RUN_NAME` 값의 check run이 표시됩니다. 기본값은
`AI Code Review`입니다. 이 check가 merge를 막게 하려면 repository ruleset 또는 branch
protection에서 `AI Code Review`를 required status check로 지정해야 합니다.
