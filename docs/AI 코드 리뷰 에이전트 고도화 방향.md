...
| 지표 | 설명 | 목표 |
| --- | --- | --- |
| Routing macro F1 | label fixture의 low/medium/manual-high class별 F1 평균 | 0.95 이상 |
| Retrieval Recall@2 / MRR@2 | 실제 prompt의 정답 정책 재현율과 첫 정답 순위 | 0.85 / 0.75 이상 |
| Citation Validity | finding 출처가 실제 retrieved source와 일치하는 비율 | 98% 이상 |
| Finding Precision@5 / Recall@5 | maintainer 지적 또는 seeded defect 재현 성능 | 0.70 / 0.60 이상 |
| Severe False-positive Rate | 승인 후 snapshot의 근거 없는 high finding 비율 | 10% 이하 |
| Secret Masking Pass Rate | secret fixture가 prompt/trace/comment에 노출되지 않는 비율 | 100% |
| Review Latency p95 | diff bucket별 check 완료 이후 agent 시간 | 75/150/300초 이하 |
