...
보인다. PR 요약은 `변경 요약`, `파일별 변경 요약`, `리뷰` 세 구역으로 제공한다. inline으로
표시된 지적도 종합 댓글의 리뷰 영역에 함께 유지하고, 재실행하면 같은 유형의 최신 결과로
기존 종합 댓글과 식별 가능한 inline 지적을 갱신한다. 사용자가
더 넓은 검토가 필요하다고 판단하면 Check 버튼을 눌러 시간·공간 복잡도,
간소화, 구조와 운영 영향을 보는 심층 리뷰를 추가한다.
사용자에게 보이는 설명은 한국어를 기본 계약으로 삼는다. 모델이 영어 summary를 반환하면 변경
통계 기반 한국어 문장으로 대체하고, message 또는 suggestion이 한국어가 아닌 finding은 게시하지
않는다. 파일 경로, 코드 식별자, API 이름과 정책 ID만 원문 표기를 유지한다.
...
- “사용자에게 보이는 요약, 리뷰와 GitHub Check 상태는 한국어 계약을 적용하고 영어 finding은 게시 전에 차단한다.”
- “공식 출처 23개를 26개 증거·오탐 지식 카드로 정제하고 모든 카드의 출처 연결을 검증한다.”
- “12개 선택 fixture에서 skill·카드·정책 유형 Recall 1.0, 기존 top-3 대비 정책 context 38.47% 감소를 확인했다.”
...
| 하네스 선택 fixture 12개 | 36개 fixture와 오픈소스 finding A/B baseline |
