...
원인 해결 중심으로 빠르게 리뷰하며 통과 PR은 관련 정책을 찾아 근거 기반 리뷰를 작성한다.
...
표준 리뷰에서는 API, 테스트, 보안, 성능 같은 내부 정책 중 변경과 관련된 항목만 검색해
Solar3 prompt에 넣는다. 모델 finding은 파일, diff line과 정책 출처를 검증한 뒤 GitHub inline
review와 PR 요약으로 보인다. 사용자가 더 넓은 검토가 필요하다고 판단하면 Check 버튼을 눌러 시간·공간 복잡도,
...
2. **근거**: 팀 정책을 검색하고 finding의 출처로 연결한다.
...
- **Finding**: severity, category, file, line, message, suggestion, policy source를 가진 지적
...
실패 check가 없을 때 자동으로 실행한다. 변경과 관련된 공통 정책을 찾아 테스트, API 계약,
스타일, 보안 등의 문제를 검토한다. 일반적인 PR의 기본 리뷰 경험이다.
...
외부에서 보면 GitHub가 사용자 인터페이스, Review Orchestrator가 판단 엔진, Policy Retrieval이
조직 지식, LLM이 분석 엔진, Postgres와 Langfuse가 기억과 관측 역할을 맡는다.
...
- **정책 검색**: Recall@3, MRR@3, unrelated retrieval rate
- **정책 근거**: finding의 citation이 실제 retrieved source와 일치하는 비율
...
- “정책은 Postgres에 저장하고 lexical retrieval로 관련 상위 3개를 사용한다.”
...
| 현재 동작 | 다음 목표 |
| --- | --- |
| 공통 정책 lexical top-3 | 저장소별 정책과 hybrid retrieval |
...
| 품질 KPI 정의 | 내부 fixture와 오픈소스 baseline 측정 |
