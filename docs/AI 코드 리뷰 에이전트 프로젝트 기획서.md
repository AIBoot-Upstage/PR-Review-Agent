...
#### 시나리오 3: 다른 관점의 심층 검토가 필요한 PR

기본 medium 리뷰가 고위험 경로나 큰 diff를 감지하면 GitHub Check에 심층 리뷰를 선택할 수 있음을 표시한다. 사용자가 `심층 리뷰 실행`을 누르면 Solar3 high reasoning으로 별도 리뷰를 수행한다. 이 경로는 기본 리뷰를 반복하지 않고 설계, 보안, 시간/공간 복잡도, 동작을 보존하는 코드 간소화, 유지보수성과 운영 리스크를 다른 관점에서 검토한다.
...
7. 에이전트가 PR 상태를 분석해 low 또는 medium 자동 경로를 선택하고, 필요하면 선택적 high 리뷰를 제안한다.
...
7. Router가 자동 리뷰의 Solar3 low 또는 medium 경로를 선택한다.
8. medium 경로와 사용자가 요청한 high 경로에서는 RAG Retriever가 관련 공통 정책 문서를 검색한다.
...
| 언어 | Python 3.11+ |
| Backend API | FastAPI |
| Agent orchestration | LangGraph StateGraph, FastAPI background task |
| LLM Gateway | LiteLLM |
| LLM Provider | Upstage Solar3 API |
| GitHub 연동 | GitHub App Webhook, GitHub REST API, GitHub Checks API |
| RAG | PostgreSQL policy chunk 저장 + lexical retrieval, pgvector 전환 준비 |
| DB | PostgreSQL |
| Queue/Cache | 현재 미사용, 운영 확장 시 durable queue 검토 |
...
| 사용자가 GitHub Check의 심층 리뷰 action 요청 | deep_quality_review | Solar3 high | 설계, 보안, 시간/공간 복잡도, 코드 간소화, 유지보수성의 다른 관점 |
...
| 영역 | 지표 | 목표 |
| --- | --- | ---: |
| 신뢰성 | 유효 webhook 수락률 | 99.5% 이상 |
| 신뢰성 | eligible PR head SHA의 자동 리뷰 완료율 | 98.0% 이상 |
| 신뢰성 | timeout/rate-limit의 10분 내 재시도 복구율 | 95.0% 이상 |
| 라우팅 | 36개 이상 label fixture의 route macro F1 | 0.95 이상 |
| RAG | 실제 prompt 정책 Retrieval Recall@2 / MRR@2 | 0.85 / 0.75 이상 |
| RAG | 실제 retrieved source와 citation 일치율 | 98% 이상 |
| 품질 | 오픈소스 maintainer finding Precision@5 / Recall@5 | 0.70 / 0.60 이상 |
| 품질 | 승인 후 snapshot의 severe false-positive rate | 10% 이하 |
| 안전 | file reference validity / secret masking pass rate | 100% / 100% |
| 비용 | 품질 하한 충족 상태에서 high-only 기준 비용 절감률 | 25% 이상 |

Latency는 `check_suite.completed` 수신부터 AI Check 완료까지 측정한다. 평균 하나를 사용하지 않고 small(`<=200 lines, <=5 files`) p95 75초, medium(`<=800 lines, <=20 files`) p95 150초, large(`<=2,000 lines, <=40 files`) p95 300초를 목표로 한다. CI 시간은 `ci_wait_ms`로 분리한다. KPI의 분모, 오픈소스 PR 데이터셋과 측정 방법은 `현재 구조 점검 및 정량 평가 계획.md`를 따른다.
...
| 4단계 | RAG 구현 | 정책 문서 indexing, lexical 검색, medium route prompt, vector 전환 schema |
| 5단계 | 선택적 high 리뷰 구현 | 복잡도/간소화 deep review prompt, 비용/latency 기록 |
...
2. 배포된 공통 정책 규칙과 컨벤션을 일관되게 적용할 수 있다.
